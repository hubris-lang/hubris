...
impl Module {
    pub fn with_name(mut s: String) -> Module {
        s.push('\0');
        Module {
            module_ref: unsafe { llvm_sys::core::LLVMModuleCreateWithName(s.as_ptr() as *const _) },
        }
    }
    ...
    pub fn print_to_file<P: AsRef<Path>>(&self, path: P) {
        ...
        unsafe {
            llvm_sys::core::LLVMPrintModuleToFile(self.as_ptr(),
    ...
    pub fn get_function(&self, mut name: String) -> Option<Function> {
        unsafe {
            ...
            let fun_ref = llvm_sys::core::LLVMGetNamedFunction(self.as_ptr(),
    ...
    pub fn set_target(&self, mut triple: String) {
        triple.push('\0');
        unsafe { llvm_sys::core::LLVMSetTarget(self.as_ptr(), triple.as_ptr() as *const _) }
    }
}
